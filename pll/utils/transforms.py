...
def get_train_transform():
    transform = [
        ...
        # Drop out
        RandomDropout(dropout_ratio=0.2, dropout_application_ratio=0.2),
        # Rotate
        RandomRotate(angle=[-1, 1], axis="z", center=[0, 0, 0], p=0.5),
        RandomRotate(angle=[-1 / 64, 1 / 64], axis="x", p=0.5),
        RandomRotate(angle=[-1 / 64, 1 / 64], axis="y", p=0.5),
        # Scale
...
def get_post_transform():
    ...
    return test_voxelize, Compose(post_transform)
...
class NormalizeColorStd(object):
    ...
    def __call__(self, data_dict, *args, **kwargs):
        if "color" in data_dict.keys():
            ...
            colors = np.squeeze(self.normalize_color(image=pseudo_image)["image"])
...
class SphereCrop(object):
    ...
    def __call__(self, data_dict, *args, **kwargs):
        ...
        target_labels = kwargs.get("target_labels", None)
        label_type = kwargs.get("label_type", "instance")
        ...
        if target_labels is not None:
            assert label_type in ["instance", "semantic"], "label_type must be 'instance' or 'semantic'"
            assert label_type in data_dict.keys(), f"{label_type} labels must exist in data_dict"
        ...
        if self.mode == "all":
            ...
            if data_dict["coord"].shape[0] > point_max:
                ...
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    if target_labels is not None:
                        target_mask = np.isin(data_dict[label_type], target_labels)
                        non_target_idx = np.argsort(dist2[~target_mask])[: point_max - target_mask.sum()]
                        idx_crop = np.concatenate([np.where(target_mask)[0], non_target_idx])
                    else:
            ...
            if target_labels is not None:
                ...
                if target_mask.sum() > 0:
                    center = data_dict["coord"][target_mask][
                        np.random.randint(data_dict["coord"][target_mask].shape[0])
                        ]
            else:
            ...
            dist2 = np.sum(np.square(data_dict["coord"] - center), 1)

            if target_labels is not None and target_mask.sum() > 0:
                target_mask = np.isin(data_dict[label_type], target_labels)
                non_target_idx = np.argsort(dist2[~target_mask])[: point_max - target_mask.sum()]
                idx_crop = np.concatenate([np.where(target_mask)[0], non_target_idx])
            else:
...
class Compose(object):
    ...
    def __call__(self, data_dict, *args, **kwargs):
        for t in self.transforms:
            data_dict = t(data_dict, *args, **kwargs)
        return data_dict
