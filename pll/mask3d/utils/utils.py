...
def load_backbone_checkpoint_with_missing_or_exsessive_keys(cfg, model):
    state_dict = torch.load(cfg.general.backbone_checkpoint, map_location=torch.device('cpu'))["state_dict"]
...
def load_checkpoint_with_missing_or_exsessive_keys(cfg, model):
    state_dict = torch.load(cfg.general.checkpoint, map_location=torch.device('cpu'))["state_dict"]
    correct_dict = dict(model.state_dict())
