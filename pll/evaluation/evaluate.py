...
import evaluation.eval_util as util_3d
...
def evaluate_inst(
    preds: dict, gt_path: str, input_CLASS_LABELS: list, input_VALID_CLASS_IDS: list, dataset: str = "scannet", 
):
    global CLASS_LABELS
    ...
    VALID_CLASS_IDS = input_VALID_CLASS_IDS
...
def instance2boxarea(points):
    return (torch.max(points[:,0])- torch.min(points[:,0])) * (torch.max(points[:,1])- torch.min(points[:,1])) * (torch.max(points[:,2])- torch.min(points[:,2]))
